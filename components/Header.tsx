...
const Header: React.FC = () => {
  ...
  const navLinkClass = ({ isActive }: { isActive: boolean }) =>
    `py-2 px-3 rounded-md text-sm font-medium transition-colors ${
      isActive ? 'bg-green-100 text-brand-green' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900'
    }`;
    
  const mobileNavLinkClass = ({ isActive }: { isActive: boolean }) =>
    `block py-2 px-4 text-base font-medium transition-colors ${
      isActive ? 'bg-green-100 text-brand-green' : 'text-gray-600 hover:bg-gray-100'
    }`;
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <div className="flex-shrink-0">
            <NavLink to="/" className="text-2xl font-bold text-brand-green">
              MeenatchiTraders
            </NavLink>
          </div>
